	...
	if(iter_table == v[iter_hash->second].end()){ // key 존재 x
		...
		*/
		...
		{// 획득한 적이 없는 경우 
			...
			if(ret_lock->lock_mode == 1 && now_table->head != ret_lock) {
				ret_lock->is_sleep=1;
				pthread_mutex_unlock(&lock_table_latch);
				return 1;// 해당 lock이 x 
			}				
			else if(now_table->head != ret_lock && (ret_lock->prev->is_sleep==1 || ret_lock->prev->lock_mode==1)) {
				ret_lock->is_sleep=1;
				pthread_mutex_unlock(&lock_table_latch);
				return 1; // 해당 lock이 share		
			}
...
void lock_wait(lock_t* ret_lock, int trx_id){
	ret_lock->is_sleep=1;
	...
	pthread_mutex_lock(&now_trx->trx_lock);
    pthread_mutex_unlock(&trx_table_latch);
	ptr_lock_table_t now_table = ret_lock->sentinel;

	// trx_mutex 획득 후 sleep
	if(ret_lock->lock_mode == 0 ) // s mode
	{
		// 자기 차례일 때까지 wait (제일 앞에 or 내 앞이 s고 깨어있음)
		while(now_table->head != ret_lock && (ret_lock->prev->is_sleep==1 || ret_lock->prev->lock_mode==1))
			pthread_cond_wait(&(ret_lock->waiting_cond), &now_trx->trx_lock);
	}
	else                          // x mode
	{
		// 자기 차례일 때까지 wait	
	...
	ret_lock->is_sleep=0;
	pthread_mutex_unlock(&now_trx->trx_lock);
	// shared 일 때 다음 share도 깨우자 
	if(ret_lock->next != NULL && ret_lock->next->lock_mode == 0 && ret_lock->lock_mode==0)
	{
		pthread_mutex_lock(&trx_table.trx_list.find(ret_lock->trx_id)->second->trx_lock);
		pthread_cond_signal(&(ret_lock->next->waiting_cond));
		pthread_mutex_unlock(&trx_table.trx_list.find(ret_lock->trx_id)->second->trx_lock);
	}
...
int lock_release(int trx_id, int is_abort)
{
	...
	for(auto iter = will_release.begin(); iter!= will_release.end(); ++iter )
	{
		...
		{// 있으면 자고 있는 거 하나 깨우기
			...
			if(now_lock != NULL){
				pthread_mutex_lock(&trx_table.trx_list.find(now_lock->trx_id)->second->trx_lock);
				pthread_cond_signal(&(now_lock->waiting_cond));
				pthread_mutex_unlock(&trx_table.trx_list.find(now_lock->trx_id)->second->trx_lock);
			}				
		}
	}
