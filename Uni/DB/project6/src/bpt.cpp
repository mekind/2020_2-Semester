...
int db_find(int table_id, int64_t key, char* ret_val, int trx_id){
	...
	if (i < Buffer[tmp].page.Num_Keys && Buffer[tmp].page.Records[i].key == key) { // 존재할 때
		ptr_lock_t ret_lock;
		int ret = lock_acquire(table_id, key, trx_id, 0, ret_lock);
	
		if(ret == 1){ // trx_lock 획득
			buffer_unpin_no_dirt(tmp); // page unlock
			lock_wait(ret_lock, trx_id); // trx_lock release
		...
		make_log_in_trx(trx_id, ret_val, ret_lock, c);
		buffer_unpin_no_dirt(tmp); // page unlock		
...
int db_update(int table_id, int64_t key, char* values, int trx_id){ 
	...
	if (i < Buffer[tmp].page.Num_Keys && Buffer[tmp].page.Records[i].key == key) { // 존재할 때
		...
		//printf("fin bpt2,%d %p\n",ret,&ret_lock);
		//uint64_t prev_LSN = Buffer[tmp].page.pLSN;
		strcpy(_old_val, Buffer[tmp].page.Records[i].value);
		...
		if (make_log_in_trx(trx_id, _old_val, ret_lock, c) == 0)
			now_trx->prev_LSN = make_update_log(now_trx->prev_LSN, trx_id, 1, table_id, c, _old_val, values, i);
		Buffer[tmp].page.pLSN = now_trx->prev_LSN;
		buffer_unpin(tmp); // page unlock
		//make_update_log()
		return SUCCESS;
	}
	else { // 존재하지 않을 때
